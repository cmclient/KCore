...
@Getter
public class CobblexManager extends Manager {

    private final List<CaseItem> items = new LinkedList<>();

    public CobblexManager() {
        this.load();
    }

    private void load() {
        File file = new File(this.core.getDataFolder(), "cobblex.yml");

        if (!file.exists()) this.core.saveResource("cobblex.yml", true);
